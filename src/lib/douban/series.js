/* global $, dui, GM_xmlhttpRequest */

import { getCkValue } from "./auth.js";
...
async function markSubjectsAsRead(ids, progress) {
    ...
    try {
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            // 更新进度（如果存在）
            try { progress.update(i + 1, ids.length); } catch { ignoreUiException(); }
            // 适当延迟，降低被限流风险
            await new Promise(r => setTimeout(r, 800));
        }
    } finally {
        if (ownProgress) try { ownProgress.close(); } catch { ignoreUiException(); }
    }
            ...
            try {
                ...
                frm.on('submit', async function (e) {
                    ...
                    for (let i = 0; i < ids.length; i++) {
                        ...
                        try { addProgress.update(i + 1, ids.length); } catch { ignoreUiException(); }
                    }
                    addProgress.close();
                    var obj = {
                        __title: "收藏丛书成功",
    ...
    try {
        ...
        for (let p = 1; p <= maxPages; p++) {
            ...
            try {
                ...
                try { progress.update(p, maxPages); } catch { ignoreUiException(); }
...
export function startDoubanSeriesEnhancement() {
    // 在 DOM 就绪后尝试注入按钮
