            ...
            return r.text().then((html) => {
              ...
              for (const node of entryNodes) {
                try {
                  ...
                  const desc = bkdesc
                    ? bkdesc.textContent.trim().replace(/\s+/g, " ")
                    : a
                      ? a.textContent.trim()
                      : "";
                  seen.add(href);
...
export function findEarliestYear(versions) {
  if (!versions || versions.length === 0) return null;
  return Math.min(...versions.map((v) => v.year));
}

export function findLatestYear(versions) {
  if (!versions || versions.length === 0) return null;
  return Math.max(...versions.map((v) => v.year));
}

export function annotateEarliestPublicationYear(earliestYear, latestYear) {
  ...
  const finalEarliest =
    currentYear && earliestYear ? Math.min(currentYear, earliestYear) : earliestYear || currentYear;
...
export async function runDoubanEarliestPublication() {
  try {
...
export function startDoubanEarliestPublication() {
