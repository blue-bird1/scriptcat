import { readSteampyGmToken, syncSteampyTokenOnSteampyPage } from "../steampy/access-token.js";
import { createSteampyXbootClient } from "../steampy/xboot-client.js";

export function startKeylolSteampyPrice() {
  "use strict";
  ...
  /**
   * 获取SteamPY的AccessToken（优先从油猴存储获取）
  ...
  function showTopNotice(message, once = true) {
    ...
    notice.style.cssText =
      "background:#fff3cd;border:1px solid #ffeeba;color:#856404;padding:8px;margin:8px;font-size:13px;";
  ...
  const getAllSteamLinks = () => {
    ...
    targetContainers.forEach((container) => {
      ...
      allSteamLinks.forEach((link) => {
        ...
        const isValid =
          appId &&
          !href.includes("store.steampowered.com/sub/") &&
          (
            (link.previousElementSibling?.href ??
              link.parentElement.previousElementSibling?.href)?.includes("barter.vg") ||
            (link.previousElementSibling?.href ??
              link.parentElement.previousElementSibling?.href)?.includes("104.236.232.190") ||
  ...
  const updatePriceDisplay = (
    ...
    if (
      ...
      !priceData.success ||
      !priceData.result ||
      !priceData.result.content ||
      priceData.result.content.length === 0
    ) {
   ...
   * @param {HTMLElement} linkElement 链接元素
   */
  const getPriceWithCacheAndSubId = async (
    appId,
    placeholder,
    linkElement,
    includeInQuote = true,
  ...
  const linkHasSteampyPriceSibling = (link) => {
    ...
    while (sibling) {
      if (
        sibling.classList.contains("steampy-key-price-container") ||
        sibling.classList.contains("steampy-key-price-placeholder")
      ) {
        return true;
      }
  ...
  const addHoverFetchForSteamLinks = () => {
    document.addEventListener(
      ...
      (event) => {
        ...
        if (!target) return;
        const link =
          (target.closest && target.closest('a[href*="store.steampowered.com/app/"]')) || null;
        if (!link) return;
