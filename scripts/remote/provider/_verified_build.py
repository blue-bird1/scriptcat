...
def clear_managed_link_stages(runtime):
    prefix = f'.current-{{build_id}}.'
    for stage in managed_stages(builds.parent, prefix, '.new'):
        if not stage.is_symlink() or os.readlink(stage) != str(runtime):
            fail('current-link migration staging path is invalid')
        stage.unlink()

def activate_current(runtime):
    current = builds.parent / 'current'
    clear_managed_link_stages(runtime)
