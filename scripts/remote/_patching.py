...
def patch_preparation_script(lock: UpstreamLock) -> tuple[str, str]:
    """Render content-addressed ScriptCat patch preparation."""
    stack = lock.patch_stacks[0]
    command = (
        'activate_patch_stack scriptcat "$build_root/src/scriptcat" '
        f"{shell_quote(lock.scriptcat.source)} {shell_quote(lock.scriptcat.commit)} "
        f'"$checkout"/{shell_quote(stack.path.as_posix())} '
        f"{shell_quote(stack.sha256)}"
    )
    helpers = r"""patch_schema='scriptcat-mcp-patch-stack-v1'
...
load_patch_series() {
  ...
  test -z "$(find "$patches" -maxdepth 1 -name '*.patch' ! -type f -print -quit)" || return 1
...
expected_patch_tree() {
  ...
  if ! GIT_INDEX_FILE="$temporary_index" git -C "$destination" read-tree "$base^{tree}"; then
    rm -f "$temporary_index"
    return 1
  fi
  for patch_file in "$@"; do
    if ! GIT_INDEX_FILE="$temporary_index" git -C "$destination" apply --cached "$patch_file"; then
      rm -f "$temporary_index"
      return 1
    fi
  done
  expected_tree=$(GIT_INDEX_FILE="$temporary_index" git -C "$destination" write-tree) || {
    rm -f "$temporary_index"
    return 1
  }
  rm -f "$temporary_index"
...
verify_patch_ref() {
  ...
  test "$(git -C "$destination" rev-parse "$patch_ref^{tree}")" = "$expected_tree" || return 1
...
ensure_patch_ref() {
  ...
  if git -C "$destination" show-ref --verify --quiet "$patch_ref"; then
    verify_patch_ref "$destination" "$patch_ref" "$desired_commit" "$expected_tree" "$base" "$signature"
    return
  fi
...
activate_patch_stack() {
  ...
  desired_commit=$(deterministic_patch_commit "$destination" "$expected_tree" "$base" "$signature")
  patch_ref="refs/scriptcat-mcp/$target/$signature"
  ensure_patch_ref "$destination" "$patch_ref" "$desired_commit" "$expected_tree" "$base" "$signature"
  checkout_is_clean "$destination" || return 1
  ...
  if [ "$current_tree" = "$expected_tree" ] && [ "$index_tree" = "$expected_tree" ]; then
