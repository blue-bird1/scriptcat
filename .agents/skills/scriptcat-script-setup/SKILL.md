...
ScriptCat MCP is a local three-stage product. Commands in `scripts/mcp/` build and test from `browser/chrome-devtools-mcp`, store build state in `~/.local/share/scriptcat-mcp-build`, create a schema 5 archive, and install it under `~/.local/share/scriptcat-mcp`. Build validates the bundled candidate runtime with a real temporary-profile shared-browser E2E. Install materializes the new release, asks the old runtime to enter an exclusive handoff while retaining abstract-socket ownership, switches `current` and `previous` transactionally, and only then commits the handoff; activation failures abort the handoff and restore the old links. A running shared broker without handoff support fails safe instead of falling back to `stop`; for that one-time upgrade boundary, end active sessions, run `scripts/mcp/control.py stop` explicitly, and retry install. The active executable remains `~/.local/share/scriptcat-mcp/current/mcp/bin/chrome-devtools-mcp.js`; all MCP publication work executes against the local checkout and local release roots. The installed shared mode uses one persistent broker for the fixed profile and provider process. Each root or subagent connects through its own stdio proxy and keeps an independent MCP session and `McpContext` while sharing the broker's browser. Closing a proxy does not end that browser lifecycle.
