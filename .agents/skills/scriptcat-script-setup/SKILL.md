...
3. When the managed directory is missing or corrupt, its manifest is invalid, or the extension remains unavailable, publish `browser/scriptcat` with `scripts/scriptcat/publish.py` to atomically repair the fixed directory. During the required publication handoff, use `shared stop`; use `shared stop --force` only when that handoff explicitly includes active sessions. Let the next proxy start a new browser lifecycle, then repeat the status check. Do not disconnect a proxy to force a refresh.
