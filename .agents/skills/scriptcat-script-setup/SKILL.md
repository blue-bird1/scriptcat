...
ScriptCat MCP is a local three-stage product. Commands in `scripts/mcp/` build and test from `browser/chrome-devtools-mcp`, store build state in `~/.local/share/scriptcat-mcp-build`, create a schema 5 archive, and install it under `~/.local/share/scriptcat-mcp`. Build validates the bundled candidate runtime with a real temporary-profile shared-browser E2E. Install materializes the new release, asks the old runtime to enter an exclusive handoff while retaining abstract-socket ownership, switches `current` and `previous`, and then commits the handoff. A successful broker commit is the irreversible activation point: later journal cleanup failures preserve the new links, while failures before commit abort the handoff and restore the old links. A running shared broker without handoff support fails safe instead of falling back to `stop`; for that one-time upgrade boundary, end active sessions, run `scripts/mcp/control.py stop` explicitly, and retry install. The active executable remains `~/.local/share/scriptcat-mcp/current/mcp/bin/chrome-devtools-mcp.js`; all MCP publication work executes against the local checkout and local release roots. The installed shared mode uses one persistent broker for the fixed profile and provider process. Each root or subagent connects through its own stdio proxy and keeps an independent MCP session and `McpContext` while sharing the broker's browser. Closing a proxy does not end that browser lifecycle.

The broker launches the provider executable with the fixed profile and receives the managed extension directory from configuration. At the start of every new browser lifecycle, it invokes trusted `Extensions.loadUnpacked` with that directory, the fixed `expectedId`, and `userScriptsAccess: true`. This atomically loads or refreshes the current directory contents and grants userscript access before the first extension service worker starts. The operation is content-driven: it does not compare or assume an extension version, and an already registered extension is still refreshed. Disconnecting or restarting a proxy does not refresh the extension because it does not start a new browser lifecycle. The MCP does not use a separate permission setter or an additional reload. The browser provider requires no change for this flow; extension, provider, and MCP publication remain independent.

## Extension Readiness

Use the managed ScriptCat tools through `mcp__chrome_devtools_scriptcat__`.

1. At the start of every MCP session, call `scriptcat_status` and confirm the extension ID, enabled state, `userScriptsAccessEnabled: true`, and service-worker readiness.
...
3. When the managed directory is missing or corrupt, its manifest is invalid, or the extension remains unavailable, publish `browser/scriptcat` with `scripts/scriptcat/publish.py` to atomically repair the fixed directory. End active MCP sessions, then run `uv run --project scripts --python 3.12 python scripts/mcp/control.py stop` as an explicit maintenance action; the command reuses the complete configured browser arguments and rejects remaining active clients. Let the next proxy start a new browser lifecycle, then repeat the status check. MCP release installation performs its own atomic handoff and never uses this maintenance stop.
