...
3. When the managed directory is missing or corrupt, its manifest is invalid, or the extension remains unavailable, publish `browser/scriptcat` with `scripts/scriptcat/publish.py` to atomically repair the fixed directory. End active MCP sessions, then run `uv run --project scripts --python 3.12 python scripts/mcp/control.py stop`; the command reuses the complete configured browser arguments and rejects remaining active clients. Let the next proxy start a new browser lifecycle, then repeat the status check.
