...
- **项目数据**：首页队列通过 `IStoreService/GetDiscoveryQueue` 取得 AppID，并通过 `IStoreBrowseService/GetItems` 载入 `StoreItem`。脚本直接读取 Steam 页面已解码的 `window.StoreItemCache`；价格、免费状态、发行日期、评论数和好评率优先复用队列数据，其中评论口径跟随 Steam 当前的评测偏好，与首页评论悬浮提示一致。缺少字段时由缓存自身的 `QueueAppRequest` 补齐，缓存不可用或字段仍缺失时才分别回退同源 `appdetails` 或 `appreviews`。经典 `?queue=1` 商品页读取当前页面展示的 `user_reviews_summary_row`：microdata `reviewCount` 提供评论数，`data-tooltip-html` 提供按整数显示的好评率。标签使用当前队列由同一 `StoreItem` 数据渲染的本地化标签文本。规则集合和自动筛选开关通过 `localStorage` 持久化，页面和队列切换后继续使用同一配置。
