...
- **项目数据**：项目 AppID 确定后，脚本请求同源 `appreviews` 与 `appdetails` 数据；评论统计和评分使用 `appreviews` 的全部语言、Steam 购买口径，与商店“所有评测”主评分一致，好评率按 `total_positive / total_reviews × 100` 计算。商店详情字段来自 `appdetails`。标签从当前 Steam 页面中该项目的真实标签元素读取，不由接口结果推断或补齐。规则集合和自动筛选开关通过 `localStorage` 持久化，页面和队列切换后继续使用同一配置。
