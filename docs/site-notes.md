...
- **项目数据**：首页队列通过 `IStoreService/GetDiscoveryQueue` 取得 AppID，并通过 `IStoreBrowseService/GetItems` 载入 `StoreItem`。脚本直接读取 Steam 页面已解码的 `window.StoreItemCache`；价格、免费状态和发行日期优先复用队列数据，缺少发行字段时由缓存自身的 `QueueAppRequest` 补齐，只有缓存不可用或字段仍缺失时才回退同源 `appdetails`。评论统计和评分继续请求 `appreviews` 的全部语言、Steam 购买口径，与商店“所有评测”主评分一致，好评率按 `total_positive / total_reviews × 100` 计算，因为实际队列 `GetItems` 未请求评论字段。标签使用当前队列由同一 `StoreItem` 数据渲染的本地化标签文本。规则集合和自动筛选开关通过 `localStorage` 持久化，页面和队列切换后继续使用同一配置。
