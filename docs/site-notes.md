...
- **新版卖家批量上架（SteamPy Plus）**
  ...
  - 普通 CD-Key 上架使用对应区域的 JSON `POST /startSell`。新版页面固定提交 `gameId`、`keys`、`keyWord`、`sellPrice`、`syncUs`；`gameId` 使用 `steamGame.id`，`keys` 按每行一个 Key 传递。批量功能以空 `keyWord` 和字符串 `"0"` 的 `syncUs` 明确关闭全球区联动，再按 `gameId` 分组串行提交；普通分组失败后继续，AccessToken 失效后停止剩余分组。鉴权沿用 SteamPy XBoot 请求头中的 `accesstoken` 与 `app_token`，响应经统一 envelope 校验后解包 `result`。
- **接口**
