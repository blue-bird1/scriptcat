...
- **商品数据契约（SteamPy Plus）**
  ...
  - 新版购买详情页 `/pro/cdKey/cdkDetail` 的公开销售表由 Vue 3 组件 `cdkDetail` 的 `columns` 和 `data` 渲染，分页、页大小和排序只替换 `data`。生产页的 `steamAva` 列仍使用 Vue 2 形式的 `h("Avatar",{props:{src}})`，Vue 3 将其渲染为空的未知 `avatar` 标签；SteamPy Plus 仅将该列的 render 替换为使用 `row.steamAva` 的标准 `img`。`listSale` 每行包含 `saleId`。SteamPy Plus 仅对已验证的 `K900` Snowflake ID 推算开始销售时间：去掉前缀后右移 22 位，再加纪元 `1524291141000` 毫秒；其他格式显示“暂无”。
