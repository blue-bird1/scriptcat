...
- **项目数据**：首页队列通过 `IStoreService/GetDiscoveryQueue` 取得 AppID，并通过 `IStoreBrowseService/GetItems` 载入 `StoreItem`。脚本直接读取 Steam 页面已解码的 `window.StoreItemCache`；价格、免费状态和发行日期优先复用队列数据，缺少发行字段时由缓存自身的 `QueueAppRequest` 补齐，只有缓存不可用或字段仍缺失时才回退同源 `appdetails`。经典 `?queue=1` 商品页读取当前页面展示的 `user_reviews_summary_row`：microdata `reviewCount` 提供该展示口径的评论数，`data-tooltip-html` 提供受 Steam 语言与评测偏好影响并按整数显示的好评率，因此经典页不再请求 `appreviews`。首页队列模态只展示评论数和评价档位，没有稳定的百分比字段；其中的评论规则统一回退 `appreviews` 的全部语言、Steam 购买口径，好评率按 `total_positive / total_reviews × 100` 计算，避免异步 DOM 与接口结果混用。标签使用当前队列由同一 `StoreItem` 数据渲染的本地化标签文本。规则集合和自动筛选开关通过 `localStorage` 持久化，页面和队列切换后继续使用同一配置。
