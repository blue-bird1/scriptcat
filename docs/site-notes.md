...
- **选择器**
  ...
  - 本地已有标注：一行一本，支持 `中文名 | 作者` 和 `中文名 | 外语名 | 作者`。书名和作者在匹配前将 Unicode 标点、符号替换为空格，并合并连续空白；书单保留原始文字。任一书名匹配且作者匹配时，在封面左上角标注绿色「本地已有」。导入会去除作者开头的 `(国家)` 或 `（国家）`，保留姓名中的括号。导入、保存和重新打开模态框均保留外语名；按中文名与清理后的作者去重，新导入优先。`---` 文件名解析继续按文字系统识别原名，并将其保留为外语名。
