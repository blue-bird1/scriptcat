...
- **MCP 本地三阶段**：MCP 使用 `browser/mcp.lock.json` 和 `scripts/mcp/`，在本地依次 build、package、install。build 在 `browser/chrome-devtools-mcp` submodule 内安装依赖、编译、打包候选 runtime，并以临时 profile 对候选 runtime 运行真实共享浏览器 E2E，只把已验证 component 保存到 `~/.local/share/scriptcat-mcp-build`；package 从该 component 生成 schema 5 归档；install 先物化新 release，再由旧 runtime 进入排他 handoff 并继续持有 abstract socket；切换 `current`/`previous` 后请求旧 broker commit。broker commit 成功是不可回滚的 activation point，之后的 journal 清理异常保留新链接；commit 前失败则 abort handoff 并恢复旧链接。broker 不存在或旧 runtime 尚无 shared 模式时继续安装；活跃客户端、运行中的旧 shared runtime 不支持 handoff 及其他控制错误均在切换前中止安装。首次从不支持 handoff 的 shared runtime 升级时，维护者须先运行 `scripts/mcp/control.py stop`，再重试 install；安装器不得自动 fallback stop。运行配置继续使用 `~/.local/share/scriptcat-mcp/current/mcp/bin/chrome-devtools-mcp.js`。
...
# ScriptCat MCP broker 管理；stop 仅用于系统退出或明确维护
