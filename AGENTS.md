...
- **运行时职责**：浏览器提供者负责定制 Chromium 的补丁、协议测试以及持续可用的浏览器接口；派生 MCP 直接消费该接口。浏览器可独立更新，只要提供者继续满足该协议。MCP 运行时只管理固定 profile 的锁、managed ScriptCat 的 ID、路径、版本、启用状态与调用错误，不协商协议，也不校验浏览器的 hash、commit、版本、release root 或路径。
