...
- **MCP lifecycle 所有权**：`chrome-devtools-scriptcat` 独占固定 profile 与 browser provider 进程的完整生命周期。Agent 不得发送进程信号、结束 Chromium 或其子进程、手动启动 provider、复制固定 profile、删除 `Singleton*`，或用其他浏览器进程争用该 profile。MCP transport 关闭时立即停止当前浏览器验收；浏览器清理与重启由 MCP/client lifecycle 完成，只在新的 lifecycle 已建立后继续，并先调用 `scriptcat_status`。
